...
```{r}
rm(list=ls())
WeeklyAge <- read_csv("./DATA/Weekly_by_Age_Sex.csv")
...
ggplot(sex_totals) + geom_col(aes(x=Sex,y=COVID19)) +
  labs(title="Total Deaths by Sex",subtitle="(As of Dec. 27, 2020)")
...
```{r}
CDCAge <- data.table::fread("./DATA/States_by_Age_Sex.csv")
CDCAge$Date <- as.Date(CDCAge$Date,format="%m-%d-%Y")
```
```{r}
CDCAge %>% filter(Age != "All Ages" & Sex != "All") %>% na.omit() %>%
ggplot() + geom_col(aes(x=Age,y=Totals/sum(Totals,na.rm=TRUE))) +
...
```{r}
ggplot(all_all) + geom_line(aes(x=Date,y=Total,col="Total Deaths")) +
...
```
```{r}
not_all_ages <- subset(all_sex, Age %in% c("5-14 Years","15-24 Years","25-34 Years","35-44 Years","45-54 eYars") & Date >="2020-03-01")
```
```{r}
ggplot(not_all_ages) + geom_line(aes(x=Date,y=COVID19,col="Deaths")) + facet_wrap(~Age,ncol=2) +
  labs(title="COVID19 Deaths by Week for Age Groups =< 55",
...
```{r}
all_age %>% filter(Date >="2021-01-16") %>%
ggplot() + geom_col(aes(x=reorder(Age,COVID19),y=COVID19)) + coord_flip() +
  labs(title="Total COVID19 Deaths by Age Group",subtitle="(Date >=2021-01-16)"
       ,caption="(Source http://www.cdc.gov)")
```
